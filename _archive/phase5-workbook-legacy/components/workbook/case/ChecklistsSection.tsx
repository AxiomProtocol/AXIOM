...
export default function ChecklistsSection({ caseId }: ChecklistsSectionProps) {
  ...
  useEffect(() => {
    ...
    const fetchProgress = async () => {
      try {
        const res = await fetch(`/api/workbook/checklists?caseId=${caseId}`);
  ...
  const toggleStep = async (checklistId: string, stepId: string) => {
    ...
    try {
      await fetch(`/api/workbook/checklists?caseId=${caseId}`, {
        method: 'POST',
