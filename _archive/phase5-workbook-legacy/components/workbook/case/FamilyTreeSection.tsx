...
export default function FamilyTreeSection({ caseId }: FamilyTreeSectionProps) {
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl max-w-lg w-full p-6 max-h-[80vh] overflow-y-auto">
            ...
            {savedRecords.length === 0 ? (
              <p className="text-gray-500 text-center py-8">No saved records to import.</p>
            ) : (
              <div className="space-y-3">
                {savedRecords.map(record => (
                  <div key={record.id} className="border rounded-lg p-3 flex items-center justify-between">
                    <div>
                      <div className="font-medium text-gray-900">{record.record_name}</div>
                    ...
                    <button
                      onClick={() => {
                        importFromRecord(record);
                        setShowImportRecords(false);
                      }}
