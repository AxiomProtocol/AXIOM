...
- **On-Chain Lending Credit Market (Task #31, v7)**: Two production smart contracts deployed and verified on Arbitrum One. `AXIOMFixedLoan` (`0x96634c2E1E80Fa51d45F0e9aB9F49B7dB3e9c859`) — fixed-term loan engine; AMORTIZED/INTEREST_ONLY modes; draw tranches (up to 3); 6 states; Maple-style grace period enforcement (`markDelinquent` requires `block.timestamp >= dueAt`; `defaultLoan` requires `block.timestamp >= dueAt + gracePeriodSeconds`); `paymentSchedule()`, `nextPaymentDue()`, `daysDelinquent()`. `AXIOMCreditMarket` (`0xeE21B3C0D89b8EfD9eD61A7FD0B98A637eA9ab37`) — ERC-3643 gated LP pool; `interestPerShare` pro-rata distribution; reserve ratio; `commitLiquidity()`, `disburseLoan()`, `receiveRepayment()`, `claimInterest()`. Security fixes applied across v6–v7: full CEI in repayLoan, correct amortization formula, nextPaymentDue interest-only fix, _settleInterest pays residual for zero-share LPs, withdrawLiquidity explicit liquid guard, closeLoan always calls releaseCommitment, grace period enforcement. UI: per-loan lifecycle panel with on-chain state badge + payment schedule table sourced from contract `paymentSchedule()`. API `loan-lifecycle.ts` single-loan GET calls on-chain `paymentSchedule()` + surfaces `dueAt`, `gracePeriodSeconds`, `defaultEligibleAt`. Config: `src/config/activeContracts.generated.ts` + `src/config/creditMarket.generated.ts`.
