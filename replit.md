...
- **Craftsman Cost Database**: `rehab_cost_benchmarks` table seeded with 57 reference costs from Craftsman National Construction Estimator (NCE). Covers 18 systems × 3 condition levels (light/medium/full_replace) × property type (sfr/multifamily/both). Cost units: per_unit, per_sqft, per_door, per_window, flat. API at `/api/rehab-costs?property_type=sfr|multifamily`. Scope generation is fully deterministic (no AI call) — see `pages/api/field-intelligence/[sessionId]/generate-scope.ts`. Regional pricing from `regional_cost_modifiers` is applied by matching the property's city (metro_areas array) and state to the best region; falls back to NATIONAL (1.0x). Atlanta=0.90x, Charlotte=0.89x, Houston=0.88x, Dallas=0.89x, Chicago=1.06x, NYC=1.30x, Boston=1.19x, SF=1.32x, LA=1.15x, Seattle=1.12x.
...
- **Cost Intelligence Engine**: Production-grade rehab underwriting engine grounded in Craftsman National Construction Estimator (NCE) cost data. Full pipeline: `cost_estimates` → `cost_estimate_scope_items` → `cost_estimate_line_items` (with version snapshots and benchmark variance tracking). Provider abstraction (`CraftsmanLocalProvider` / `CraftsmanHttpProvider`) allows future API swap. 8 system templates (light cosmetic, unit turn, medium value-add, heavy gut, kitchen, bathroom, exterior, MF systems). Regional cost modifiers for 14 metros (Atlanta 0.90x, Houston 0.88x, NYC 1.30x). Integrated as "Cost Intelligence" tab in deal workspace. APIs at `/api/cost-intelligence/estimates`, `/catalog`, `/templates`, `/benchmarks`. Service layer at `server/services/cost-intelligence/`. Types at `lib/cost-intelligence/types.ts`. Docs at `docs/cost-intelligence.md`.
