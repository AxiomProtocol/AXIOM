...
Wallet connection utilizes Wagmi v2.14 + RainbowKit v2.1.7 with SIWE (Sign-In With Ethereum) for authentication, configured for the Arbitrum One chain. The wagmi config explicitly specifies a wallet list (MetaMask, WalletConnect, Coinbase, Rainbow, Injected) to avoid pulling in connectors with uninstalled optional dependencies. Key files: `lib/web3/wagmiConfig.ts` (config + wallet list), `components/WalletConnect/WalletContext.tsx` (main context with Wagmi hooks + auto-SIWE), `components/design-law/ConnectWalletButton.tsx` (RainbowKit ConnectButton.Custom), `lib/web3/useWallet.ts` (legacy backward-compat wrapper), `pages/_app.js` (WagmiProvider + QueryClientProvider + RainbowKitProvider + WalletProvider stack). Webpack aliases in next.config.js were removed — no longer needed with wagmi v2 connectors.
...
- **Fiat On-Ramp** (Planned): Placeholder for fiat on-ramp provider evaluation.
