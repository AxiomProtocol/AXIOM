...
### Deployment
Production deployment uses Replit VM (Reserved VM) — always-running persistent server with a stable outbound IP. This is required for BitGo's IP-restricted API tokens and eliminates serverless cold-start issues. Build: `npm run build` (`next build`). Run: `npm run start` (`next start -H 0.0.0.0 -p 5000`). Production port: 5000 (same as dev, mapped to external port 80). Custom domain: `axiomprotocol.app`. The in-memory rate limiter (`lib/rateLimit.ts`) works correctly because the process stays running (would not work in serverless/autoscale).
