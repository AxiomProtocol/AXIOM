...
- **Physical-Digital Bridge (Land Acquisition Pipeline)**: Visualizes the full land acquisition lifecycle.
- **Property Analysis Tool**: A pay-per-report property analysis tool at `/property`.
...
- **ERC-3643 Unified AXUSD (T-REX Compliant)**: A unified ERC-3643 compliant stablecoin replacing the dual GENIUS/Legacy AXUSD ecosystem. 13 contracts deployed and verified on Arbitrum One. Frontend dashboard at `/axusd-3643` with 4 tabs (Overview, Identity, Compliance, Contracts) — all data fetched live from on-chain via `/api/erc3643/dashboard`. Backend services: `lib/services/ERC3643Service.ts` (identity, compliance, claims, freeze, platform whitelisting), `lib/services/IdentityBridgeService.ts` (legacy KYC bridge). API routes: `/api/erc3643/dashboard` (aggregated token+compliance+identity data), `/api/erc3643/identity/status|register|claim`, `/api/erc3643/compliance/check|modules`, `/api/erc3643/admin/freeze|platform`. Contract config: `shared/contracts-3643.ts`. Schema: `shared/erc3643Schema.ts` (t3_ tables). Old `/axusd` page has migration banner linking to `/axusd-3643`. Disclosure page updated with new contract addresses; legacy AXUSD contracts marked deprecated.
