...
### UI/UX Decisions
The frontend features a modular, responsive design with a professional gold/black theme and yellow accents, and also uses white backgrounds with teal, purple, and gold accents. Branding includes "AXIOM" with a golden circular token logo and the tagline "Build Wealth Together, On-Chain." A unified navigation system uses a land-first approach with consistent headers and footers. The official Web3 design template provides an immersive experience with glassmorphism, 3D depth, and scroll animations.
...
- On-chain GovernanceHub with 24h timelock, role-based access, and emergency pause
...
- Axiom SUSU (Rotating Savings Groups) with Community Pool and Personal Vault custody
...
- SEED (Wealth Engine) for AXM locking for voting power and participation benefits
...
- DSCR Governance & Reporting System (4-phase institutional-grade transparency system for SEC Reg D 506(c) compliance)
...
- Node Operator Program (Comprehensive operator lifecycle management with 6-phase onboarding; role-gated attestations; USD-denominated AXIOM compensation with milestone-based accrual; dual attestation workflow; incident reporting with slashing schedule; weekly transparency metrics. Operator Portal at `/operator` with SIWE wallet authentication.)
- Note Acquisition Pipeline (Mortgage note sourcing and research portal at `/notes/pipeline` and `/notes/submit`; 6-stage workflow; dual attestation integration with node operators via `/api/notes/attest`; PostgreSQL-backed with comprehensive input validation; server-side LTV/discount calculation; performance status filtering.)
- Euler V2 AXUSD Lending Markets (External DeFi lending integration on Arbitrum One at vault 0xe3048078286eA27fF91Eed10AA5FD749F0Ce7059 with eUSDC/eWETH collateral support, integrated via `/earn` page, `/borrow` page, DEX Earn tab, yield-vault page, and dashboard widget. **LOCKED CONFIG** - DO NOT CHANGE vault address or LTV parameters.)
- National Economic Pilot ($1M dual-asset investment tracking system at `/pilot` with 8 subpages: dashboard, investors, distributions, reports, documents, projections, performance, audit. 14 PostgreSQL tables, 17 API endpoints, NotificationService with Resend email integration. Two SPVs: Cash Flow Anchor ($600K multifamily) and Appreciation Asset ($350K commercial/industrial). 35/35/20/10 treasury allocation policy. Phase 2 expansion gate scoring. Uses raw SQL via pg.Pool for API routes.)
