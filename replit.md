...
- **Member Profile System** (`/profile`, `/profile/[wallet]`): Comprehensive personal profile pages with social sharing capabilities. Features include banner images, profile photos, headline/tagline, bio, purpose statement, occupation, skills tags, location, website links, and member tier badges (Explorer/Builder/Leader). Profiles support OG meta tags for Twitter, Facebook, LinkedIn, and WhatsApp sharing. Profile fields: `headline`, `banner_image_url`, `member_tier` (explorer/builder/leader), `member_since`, `total_groups_joined`, `total_savings_contributions`, `courses_completed`, `referral_code`, `profile_visibility`. API at `/api/profile/[wallet]` supports lookup by wallet address or username. Profile index page at `/profile` redirects connected users to their own profile.
- **3-Step Journey Framework**: Main navigation restructured around a Learn → Connect → Save Together journey. Step Progress Banner (`components/StepProgressBanner.js`) shows contextual progress across pages. Navigation centralized in `lib/navigation.js` with items: Start, Learn, Connect, Save Together, Grow, Advanced, About.
