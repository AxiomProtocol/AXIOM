...
**Solvency and Reserve Transparency:** The `/solvency` page is a three-mode institutional solvency console with Allocator, Clearinghouse, and Regulatory views. It combines live metrics from a database-backed snapshot system (`solvencySnapshots` table) with institutional disclosure content. The **Allocator view** shows capital adequacy metrics, asset composition, and the capital waterfall. The **Clearinghouse view** provides AXUSD stability assessment, deterministic stress test scenarios (5 predefined: Market Correction, Liquidity Crisis, Black Swan, Stablecoin Depeg, Governance Attack), and historical coverage charts via recharts. The **Regulatory view** contains compliance definitions, stabilization policy modes, data integrity verification, FAQ, and full risk disclosure. APIs: `/api/solvency/metrics` (v1 contract), `/api/solvency/latest` (computed model + AXUSD stability), `/api/solvency/history` (paginated historical points), `/api/solvency/scenario` (POST stress tests, stored in `scenarioRuns` table), `/api/solvency/ingest-snapshot` (admin-protected ingestion). The math library at `lib/solvency/` provides pure-function computations for coverage ratios, reserve ratios, policy mode determination, liquidity haircuts, and stress scenario modeling. Database tables: `solvency_snapshots`, `scenario_runs`, `disclosure_events`.
