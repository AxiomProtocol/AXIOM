...
- **Document Ingestion & Extraction**: AI-powered document analysis in the Deal Intelligence workspace (Documents tab). Supports rent rolls, offering memorandums, property reports, appraisals, operating statements (T-12), insurance declarations, and lease abstracts. Uploads PDF/image/text files, extracts structured data via Gemini, and auto-maps extracted fields to deal assumptions. Schema: `shared/docExtractionSchema.ts` (doc_extractions, doc_extraction_fields). Templates: `lib/doc-extraction/templates.ts`. Engine: `lib/doc-extraction/engine.ts`. API: `/api/doc-extraction/upload|list|[id]`.
