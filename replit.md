...
**Schema file architecture**: The main `shared/schema.ts` (9600+ lines) has a CJS export limit where Next.js webpack only loads ~32 top-level exports. New table groups MUST use separate schema files to avoid silent export failures: `shared/distressedFeedSchema.ts` (dp_ tables), `shared/agentGovSchema.ts` (ag_ tables + sentinel regime snapshots mirror). Always import new tables from their dedicated schema files, not from `shared/schema.ts`.
...
- **Distressed Property Feed (Deal Flow)**: A deal sourcing pipeline at `/distressed-feed` aggregating distressed properties from government sources (HUD, Fannie Mae, Freddie Mac, USDA) and a wholesaler submission portal. Three-tab UI: Property Feed (filterable/sortable grid), Submit Deal (public wholesaler portal), My Buy Box (investor criteria matching). Database tables: `dp_listings`, `dp_buy_boxes`, `dp_matches`, `dp_wholesaler_submissions` in separate schema file `shared/distressedFeedSchema.ts`. Ingestion services in `lib/distressed-feed/`. Target states: GA, TX, NC, MS, AL, TN, SC, FL. Wholesaler submissions auto-promote to listings on admin approval.
- **Agent Governance System**: Policy-based autonomous agent authorization at `/api/agent-gov/`. Agents register with mode (ADVISORY/CONSTRAINED), policies define regime-aware rules with budget constraints, intents are evaluated against active policies before execution. Full audit trail with hash-chain verification. Database tables: `ag_agents`, `ag_policies`, `ag_budgets`, `ag_intents`, `ag_decisions`, `ag_executions`, `ag_audit_log` in separate schema file `shared/agentGovSchema.ts`. Auth via `x-admin-key` header (ADMIN_SOLVENCY_KEY). Routes: agents CRUD, policies CRUD + activate, budgets CRUD, intents + evaluate, regimes/current, audit + verify, health.
