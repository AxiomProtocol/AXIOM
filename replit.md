...
- **Axiom Sentinel**: A unified capital decision and risk authorization layer converting MIRDT signals into cryptographically auditable capital actions. It uses an in-app Next.js service as a control plane, Drizzle+PostgreSQL for data, and mixed on-chain/off-chain gating.
...
- **AME AI Oracle**: A Gemini-powered interpretation layer for AME metrics, providing institutional-grade analysis for regime narration, stress recommendations, tradeoff analysis, audit summaries, and full briefings, accessible via `/api/solvency/ame/oracle`.
- **MIRDT Execution Model**: A deterministic, auditable engine for paper trading based on MIRDT market intelligence setups, processing through a pipeline of price fetching, direction inference, classification, sizing, and decision storage. Includes a hybrid exit system with ATR-based volatility-aware riskStop (adaptive k from volRatio tiers) separating capital protection (riskStop) from thesis invalidation (invalidationLevel). Exit badges: EXIT_RISK, TAKE_PROFIT, INVALIDATED, TIME_EXIT, HOLD. Live prices via `/api/prices` (15s polling), volatility via `/api/volatility` (6hr cache, ATR(14) from AlphaVantage/CoinGecko). Core logic in `lib/mirdt/hybridExit.ts`.
- **Proof of Execution Playbook**: Internal dashboard at `/founder-ops/proof-of-execution` tracking paper trading P&L toward a $100/30-day target. Shows daily aggregates, execution metrics (win rate, Sharpe, drawdown, profit factor), full audit trail linking Setup → Decision → Trade → P&L, portfolio config, and execution run history. API at `/api/mirdt/execution/playbook`. Serves as source of truth for system capability demonstration.
- **The Wealth Practice (Group Economics Core Engine)**: The `/wealth-practice` page manages community group economics with a three-stage trust pipeline (Interest Hub → Purpose Group → On-Chain Pool) and capital flow bridge to land acquisition.
...
- **Graduated Execution Framework (GEF)**: A behavior-based qualification system at `/execution` enabling users to progress from paper trading to live execution through measurable metrics. Core components: Behavior Qualification Engine (BQE) computing 6 scores (RBAR, DSI, PSC, VRS, EDS, RCS) into a weighted EQS; 4-tier progression ladder (PAPER → TIER_1 → TIER_2 → TIER_3) with AXM commitment requirements; policy-gated intent generation with AXUSD risk budgets; transactional paper execution with MAE/MFE tracking; SHA-256 audit hash chain for tamper-evident event logging. Database: 9 `gef_` prefixed tables. API: `/api/execution/*` endpoints (admin-key protected). Live execution disabled by default (EXECUTION_LIVE_ENABLED env var).
