...
export async function ingestPrice(
  ...
  // Collateral Risk Policy §6 emergency trigger: oracle staleness on
  // ingest. If an inbound observation is *already* older than the
  // per-asset staleness budget at the moment we accept it, that is a
  // direct signal that the upstream feed is delayed — route through
  // the integrity chokepoint so the asset auto-downgrades to RED.
  // We still persist the snapshot (it is the truthful record of what
  // the upstream sent us), but we also raise the integrity event.
  // Best-effort and idempotent: chokepoint no-ops when already RED;
  // never let an integrity-write failure break the ingest path.
  if (ageSec > profile.staleSec) {
    try {
      const { recordIntegrityFailure } = await import('./risk/integrity');
      await recordIntegrityFailure({
        assetId: input.assetId,
        kind: 'oracle_stale',
        detail: `ingested ${input.priceType} from ${input.source} aged ${ageSec}s exceeds budget ${profile.staleSec}s`,
        actor,
        correlationId,
      });
    } catch (err) {
      console.error(
        '[capinfra.marketData] ingest-time integrity downgrade failed for asset',
        input.assetId,
        err,
      );
    }
...
export async function getLatestPrice(
  ...
  const isStale = ageSec > budget;

  // Collateral Risk Policy §6 emergency trigger: oracle staleness.
  // When the latest snapshot is older than the per-asset staleness
  // budget, route through the integrity chokepoint so the asset
  // auto-downgrades to RED. Best-effort and idempotent: the chokepoint
  // is a no-op if the asset is already RED, and we never let an
  // integrity-write failure break a price read. Imported lazily to
  // avoid a circular dependency with policy.ts.
  if (isStale) {
    try {
      const { recordIntegrityFailure } = await import('./risk/integrity');
      await recordIntegrityFailure({
        assetId: row.assetId,
        kind: 'oracle_stale',
        detail: `latest ${row.priceType} from ${row.source} aged ${ageSec}s exceeds budget ${budget}s`,
        actor: 'system:marketData.getLatestPrice',
      });
    } catch (err) {
      console.error(
        '[capinfra.marketData] integrity downgrade failed for asset',
        row.assetId,
        err,
      );
    }
