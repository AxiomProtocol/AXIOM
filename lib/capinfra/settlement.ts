...
// ── Webhook processor helper + notification context builder ─────

export async function getInstructionsByExternalRef(
  externalRef: string,
  adapterKind?: string,
): Promise<CapSettlementInstruction[]> {
  const conditions = [eq(capSettlementInstructions.externalRef, externalRef)];
  if (adapterKind) {
    conditions.push(eq(capSettlementInstructions.adapterId, adapterKind));
  }
  return db
    .select()
    .from(capSettlementInstructions)
