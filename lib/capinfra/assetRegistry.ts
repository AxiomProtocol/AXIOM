...
export async function createAsset(
  ...
  const row: NewCapAsset = {
    ...
    // Fail-closed: any newly-onboarded asset starts as RED collateral.
    // Re-classification to GREEN/YELLOW must be an explicit, audited update
    // with a recorded rationale (see Collateral Risk Policy §2).
    collateralClass: parsed.collateralClass ?? 'RED',
    collateralClassificationRationale: parsed.collateralClassificationRationale ?? null,
...
export async function updateAsset(
  ...
  if (parsed.collateralClass !== undefined) patch.collateralClass = parsed.collateralClass;
  if (parsed.collateralClassificationRationale !== undefined) {
    patch.collateralClassificationRationale = parsed.collateralClassificationRationale;
  }
