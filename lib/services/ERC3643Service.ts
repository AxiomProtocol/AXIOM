...
/**
 * Submit a contract method that may collide with another in-flight tx using the
 * same signer (the deployer EOA is shared with background services). Retries on
 * `NONCE_EXPIRED` / `REPLACEMENT_UNDERPRICED` by re-fetching the latest pending
 * nonce and resubmitting. Bounded so we never spin forever.
 */
async function sendWithNonceRetry(
  signer: ethers.Wallet,
  send: (overrides: ethers.Overrides) => Promise<ethers.ContractTransactionResponse>,
  label: string,
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      const collision =
        ...
        /nonce too low|already known|replacement transaction underpriced/i.test(msg);
      if (!collision || attempt === MAX_ATTEMPTS) throw e;
      const backoff = 500 * attempt;
      console.warn(`[sendWithNonceRetry:${label}] attempt ${attempt} hit nonce collision (${code ?? 'unknown'}); retrying in ${backoff}ms`);
...
export class ERC3643Service {
  ...
  static async registerIdentity(wallet: string, countryCode: number = 840) {
    ...
    // F5 fix: pass deployer (claim issuer ops EOA) as the initial MANAGEMENT_KEY on the
    // user's ONCHAINID. This is required so issueClaim() can call identity.addClaim()
    // on-chain (which is gated by onlyClaimKey: MANAGEMENT_KEY or CLAIM_SIGNER_KEY).
    // The user wallet retains all transfer/access rights via IdentityRegistry — only
    // claim-key authority on the ONCHAINID lives with the deployer (institutional KYC ops).
    const tx = await sendWithNonceRetry(
      signer,
      (overrides) => factory.createIdentity(wallet, deployerAddress, overrides),
      'createIdentity',
    );
    const receipt = await tx.wait();

    // Derive the new identity address from the IdentityCreated event in the receipt.
    // We deliberately avoid a follow-up `factory.getIdentity(wallet)` view call here:
    // load-balanced RPCs can route the read to a node that has not yet processed the
    // block, returning 0x0 and breaking the subsequent registry.registerIdentity()
    // call with a ZERO_IDENTITY revert.
    //   event IdentityCreated(address indexed wallet, address indexed identity)
    const IDENTITY_CREATED_TOPIC = ethers.id('IdentityCreated(address,address)');
    let identityAddr: string | null = null;
    for (const log of receipt?.logs ?? []) {
      if (
        log.address.toLowerCase() === ERC3643_CONTRACTS.IDENTITY_FACTORY.toLowerCase() &&
        log.topics[0] === IDENTITY_CREATED_TOPIC &&
        log.topics[1] &&
        log.topics[2]
      ) {
        const eventWallet = ethers.getAddress('0x' + log.topics[1].slice(26));
        if (eventWallet.toLowerCase() === wallet.toLowerCase()) {
          identityAddr = ethers.getAddress('0x' + log.topics[2].slice(26));
          break;
        }
      }
    }
    if (!identityAddr || identityAddr === ethers.ZeroAddress) {
      // Fallback: query the factory in case the event scan missed it for any reason.
      identityAddr = await factory.getIdentity(wallet);
    }
    if (!identityAddr || identityAddr === ethers.ZeroAddress) {
      throw new Error(
        `IdentityFactory.createIdentity tx ${tx.hash} confirmed but no IdentityCreated event found for wallet ${wallet}`,
      );
    }
    ...
    const regTx = await sendWithNonceRetry(
      signer,
      (overrides) => registry.registerIdentity(wallet, identityAddr, countryCode, overrides),
  ...
  private static async _buildAndInsertClaim(
    ...
    // F1 fix: anchor the claim on-chain via identity.addClaim() so
    // IdentityRegistry.isVerified() — which iterates ClaimTopicsRegistry topics and
    // calls IIdentity(identity).getClaimIdsByTopic(topic) — can actually find them.
    // Off-chain signatures alone are NOT sufficient: T-REX requires the claim record
    // to exist on the user's ONCHAINID; the signature is then validated by ClaimIssuer.
    // Caller must hold MANAGEMENT_KEY or CLAIM_SIGNER_KEY on the identity (see F5 fix
    // in registerIdentity() — deployer is wired as MANAGEMENT_KEY at identity creation).
    let addClaimTxHash: string | undefined;
    try {
      const identity = new ethers.Contract(
        identityRecord.onchainIdAddress,
        IDENTITY_ABI,
        signer,
      );
      const onchainTx = await sendWithNonceRetry(
        signer as ethers.Wallet,
        (overrides) => identity.addClaim(
          topic,
          1, // scheme = ECDSA
        ...
        `addClaim:topic${topic}`,
      );
      const receipt = await onchainTx.wait();
      addClaimTxHash = receipt?.hash ?? onchainTx.hash;
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(`On-chain identity.addClaim failed for topic ${topic}: ${msg}`);
    }
    ...
    return { inserted, expiresAt, refreshRequiredBy, signature, addClaimTxHash };
  }
  ...
  static async issueClaim(wallet: string, topic: number, data: string = '', opts?: { tx?: DbTx }) {
    ...
    const { inserted, expiresAt, refreshRequiredBy, signature, addClaimTxHash } = opts?.tx
  ...
  static async atomicKycApproval(params: {
    ...
    let t1AddClaimTxHash: string | undefined;
    let t3AddClaimTxHash: string | undefined;
    ...
    try {
      ...
      t1AddClaimTxHash = txResult.t1.txHash;
      t3AddClaimTxHash = txResult.t3.txHash;
    } catch (txErr: unknown) {
    ...
    return {
      ...
      t1Claim: { claimId: t1Claim.id, expiresAt: t1ExpiresAt, addClaimTxHash: t1AddClaimTxHash },
      t3Claim: { claimId: t3Claim.id, expiresAt: t3ExpiresAt, addClaimTxHash: t3AddClaimTxHash },
    };
  }
