 ...
 *
 * Canonical endpoint for the Reserves tab "Trigger Mint from PAXG" button.
 * (Companion route: /api/founder/top-up-axau-buffer shares the same logic.)
 *
 * Safety gates (checked in order):
 *  1. oracle staleness — rejects if Chainlink XAU/USD > ORACLE_STALE_THRESHOLD_SECONDS old
 *  2. mintPaused      — rejects if MintRedeemController.mintPaused is true
 *  3. balance         — rejects if deployer PAXG balance < requested amount
 *  4. cap             — paxgAmountFloat must be <= 100 PAXG per call
 *
  ...
  try {
    ...
    if (ageSeconds > ORACLE_STALE_THRESHOLD_SECONDS) {
      return res.status(409).json({
        error: `Oracle price stale — mint paused. XAU/USD last updated ${Math.floor(ageSeconds / 3600)}h ago (threshold: ${ORACLE_STALE_THRESHOLD_SECONDS / 3600}h).`,
    ...
    if (mintPaused) {
      return res.status(409).json({
        error: 'MintRedeemController.mintPaused is true — mint not available while paused',
      });
    }
    ...
    // ── 4. Pre-quote (non-blocking; informational only) ────────────────────
    ...
    console.error('[mint-axau] error:', e?.message);
    return res.status(500).json({ error: e?.message ?? 'AXAU mint failed' });
  }
}
