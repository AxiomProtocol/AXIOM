...
import { computeProbabilityModel } from '../../../lib/ivcee/models';
import { upsertProbabilityModel } from '../../../lib/ivcee/persist';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const result = computeProbabilityModel(input);
    await upsertProbabilityModel(dealId, scenarioId, result);

    return res.status(200).json({ data: result, meta: { dealId, scenarioId, computedAt: new Date().toISOString() } });
  } catch (err: any) {
    console.error('[IVCEE probability]', err.message);
