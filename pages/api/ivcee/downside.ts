...
import { computeDownsideMetrics } from '../../../lib/ivcee/models';
import { upsertDownsideMetrics } from '../../../lib/ivcee/persist';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const result = computeDownsideMetrics(input);
    await upsertDownsideMetrics(dealId, scenarioId, result);

    return res.status(200).json({ data: result, meta: { dealId, scenarioId, computedAt: new Date().toISOString() } });
  } catch (err: any) {
    console.error('[IVCEE downside]', err.message);
