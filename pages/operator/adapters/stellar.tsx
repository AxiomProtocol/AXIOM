...
export default function StellarAdapterPage({ health, recent, reconRuns, latestDrift, loadError }: Props) {
  ...
  async function runRecon() {
    setBusy(true);
    try {
      const res = await fetch('/api/capinfra/adapters/stellar/reconcile', {
        method: 'POST',
      ...
      <div className="py-8">
        ...
        <section className="border border-dl-border p-4 mb-6">
          <div className="flex justify-between items-center mb-3">
            ...
            <button onClick={runRecon} disabled={busy} className="text-xs uppercase tracking-wide border border-dl-border px-3 py-1 disabled:opacity-50">
            ...
            <>
              ...
              <table className="w-full text-xs">
                <thead><tr>
                  ...
                  <th className="text-left font-mono">Ref</th>
                </tr></thead>
                <tbody>
                  {latestDrift.map((d) => (
                    <tr key={d.id} className="border-t border-dl-border align-top">
                      ...
                      <td>
                        <span className={`text-[10px] border px-1 ${severityBadge[d.severity] ?? ''}`}>{d.severity}</span>
                      </td>
                      <td className="font-mono text-xs break-all">{d.externalRef ?? '—'}</td>
                      <td className="font-mono">{d.remediation}</td>
                      <td className="font-mono text-xs break-all">
                        {d.remediationRef ?? (d.remediationFailureJson ? `FAILED: ${JSON.stringify(d.remediationFailureJson).slice(0, 60)}` : '—')}
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <section className="border border-dl-border p-4">
          ...
          {recent.length === 0 ? (
            <p className="text-sm text-dl-muted">No Stellar webhook events recorded yet.</p>
          ) : (
            <table className="w-full text-xs">
              <thead><tr>
              ...
              <tbody>
                {recent.map((r) => (
                  <tr key={r.id} className="border-t border-dl-border align-top">
                    <td className="font-mono">{r.receivedAt}</td>
