...
contract AxiomPriceOracleV2 {
    string public constant name = "Axiom Price Oracle V2";
    ...
    function getQuote(
        ...
        // Formula: outAmount = inAmount * basePrice / quotePrice * 10^(quoteDecimals - baseDecimals)
        // Rearranged to avoid overflow: 
        // outAmount = inAmount * basePrice * 10^quoteDecimals / (quotePrice * 10^baseDecimals)
        
        if (quoteDecimals >= baseDecimals) {
            uint256 decimalDiff = quoteDecimals - baseDecimals;
            outAmount = (inAmount * basePrice * (10 ** decimalDiff)) / quotePrice;
        } else {
            uint256 decimalDiff = baseDecimals - quoteDecimals;
            outAmount = (inAmount * basePrice) / (quotePrice * (10 ** decimalDiff));
        }
    }
    ...
    function _getPrice(address token) internal view returns (uint256) {
        ...
        if (token == USDY) {
            return 103e6; // 1.03 USD with 8 decimals
        }
    ...
    function _getDecimals(address token) internal view returns (uint8) {
        ...
        if (token == WETH || token == ARB || token == USDY || token == AXUSD) {
            return 18;
        }
        return 18;
    }
