 ...
 * @notice ERC-3643-compliant AXUSD stablecoin for Polygon PoS.
 *
 * This contract is the Polygon PoS variant of the Axiom ERC-3643 suite.
 * Polygon's strategic role: payments / enterprise settlement / treasury routing.
 *
 * Carries all audit hardening from the Avalanche mainnet version:
 *
 *   A1/A2 — forcedTransfer() has nonReentrant. The freeze-clear/restore
 ...
 *            is cleared. nonReentrant closes this window.
 *
 *   A3     — Constructor does not grant MINTER_ROLE, BURNER_ROLE, or AGENT_ROLE
 *            to the deployer EOA. Only DEFAULT_ADMIN_ROLE is granted at
 *            construction. Operational roles must be explicitly granted
 *            post-deploy to the protocol multisig.
 *
 *   A5     — CountryAllowModule.allowAll is set per operator decision at deploy.
 *            Deploy script documents the allowAll setting used.
 *
 *   A6     — Module Ownable ownership should be transferred to the protocol
 *            multisig via the post-deploy script before any user activity.
 */
