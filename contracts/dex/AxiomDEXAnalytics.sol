    ...
    UUPSUpgradeable
{
    ...
    function getGlobalMetrics() external view returns (
        ...
        (totalTVL, activePools) = _calculateTotalTVL();
        totalVolume24h = _calculateTotalVolume24h();
        totalFees24h = _calculateTotalFees24h();
        totalSwaps = IAxiomExchangeHubAnalytics(exchangeHub).totalSwaps();
        historicalVolume = totalHistoricalVolume;
        historicalFees = totalHistoricalFees;
    }
    ...
    function getPoolMetrics(uint256 poolId) external view returns (
        ...
        (address tokenA, address tokenB, uint256 _reserveA, uint256 _reserveB, 
         uint256 _totalLiquidity,, bool isActive) = IAxiomExchangeHubAnalytics(exchangeHub).getPoolCore(poolId);
        
        if (!isActive) return (0, 0, 0, 0, 0, 0, 0);

        reserveA = _reserveA;
        reserveB = _reserveB;
        totalLiquidity = _totalLiquidity;
        tvl = _calculatePoolTVL(tokenA, tokenB, reserveA, reserveB);
        volume24h = poolVolumeLast24h[poolId];
        fees24h = poolFeesLast24h[poolId];
        apr = _calculateAPR(tvl, fees24h);
    }
