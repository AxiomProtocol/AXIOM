...
Tracker status: SILVER SLEEVE PATH — DEPLOYMENT READY (pending Gnosis Safe execution) — Oracle CLOSED · Regulatory CLOSED · Audit CLOSED · KAG bridge path CONFIRMED · AXM vote WAIVED
Source packet: documents/commodities/AXAG_STAGE_2_DILIGENCE_PACKET.md
Effective date: 2026-05-01
Last updated: 2026-05-02 (update 2) — **AUDIT + GOVERNANCE WAIVER**: (5) Internal smart contract audit complete (AXAG-AUDIT-001) — `documents/commodities/AXAG_INTERNAL_AUDIT_REPORT.md`. 8 findings: 1 BLOCKER (NAVEngine oracle staleness mismatch — resolved in playbook as Step 2.5: call `NAVEngine.setOracleStaleSecs(97200)` before registering silver component), 1 MEDIUM accepted (same as AXGoldVault production), 1 LOW remediated (F-03 NatSpec), 1 LOW accepted, 4 INFO. Both contracts cleared for deployment. (6) AXM token holder governance vote waived — silver sleeve is an operational collateral addition by Gnosis Safe, not a protocol change requiring token holder ratification (see AXAG-AUDIT-001 § 5). (7) Remaining open gates: G-01 Gnosis Safe quorum, G-03 KAG bridge execution, G-04a/b KAG decimals and fee-on-transfer verification, G-06 reserve KAG acquisition, G-07 disclosure flip. Previous update: O-01–O-04 CLOSED, REG-01–REG-04 CLOSED, bridge path confirmed. AXAG remains NOT LIVE AND NOT ISSUED.
...
| C-02    | Custody    | Custody RFP responses tabulated against framework Section 6 criteria          | Treasury operations lead   | Axiom Treasury Lead  | IN PROGRESS   | documents/commodities/AXAG_CUSTODY_SHORTLIST.md (live); documents/commodities/AXAG_CUSTODY_EVALUATION_MATRIX.md (live); documents/commodities/AXAG_RFP_RESPONSE_TRACKER.md (live); documents/commodities/AXAG_STAGE_2_OPTIONS_PACKAGE.md (strategic framework) | **RFP response tracker initialized; awaiting external responses.** Evaluation matrix prepared (v1.0 — public information basis). 10-dimension scoring (all 6 candidates), 16 open questions, 7 Minimum Custody Standards, 8 C-03 precondition gates. Outreach templates ready (AXAG_CUSTODY_OUTREACH_TEMPLATES.md). Options package complete: Path B immediate, Path A strategic, Hybrid path, decision tree, redemption models R1–R4. Formal RFP responses pending from: Brink's, Loomis, BitGo, Anchorage, Malca-Amit, Via Mat. |             | 2026-05-01   |
| C-03    | Custody    | Selected custodian identified (Path A, B, or documented fallback)             | Treasury operations lead   | Axiom Treasury Lead  | ASSIGNED      |                                                        | Blocked on C-02. **PRIMARY BOTTLENECK — see Section 11.** **SELECTION BLOCKED UNTIL at least one candidate satisfies MC-01 through MC-07 (Minimum Custody Standards) AND all eight preconditions PRE-01 through PRE-08 are MET** — per AXAG_CUSTODY_EVALUATION_MATRIX.md Section 3 and Section 9, and AXAG_RFP_RESPONSE_TRACKER.md Group 3. Current PRE gate: NOT MET (0/8). Options framework: see AXAG_STAGE_2_OPTIONS_PACKAGE.md for Path B (immediate), Path A (strategic), Hybrid, decision tree, and no-go conditions. |             | 2026-05-01   |
...
| L-03    | Liquidity  | Redemption SLA draft (T+0 / T+1 / T+N, identity verification, fee schedule, cancellation policy) | Liquidity operations lead   | Axiom Liquidity Lead  | ASSIGNED      | documents/commodities/AXAG_STAGE_2_OPTIONS_PACKAGE.md Section E (redemption model options R1–R4) | **Redemption model selection is required before the Liquidity Risk score can move from 3 to 2.** Four models evaluated in options package: R1 (large-lot physical — HIGH fit, low regulatory risk), R2 (custodian-directed AXUSD — MEDIUM fit, legal review required), R3 (queue + scheduled settlement — MEDIUM fit, legal review recommended), R4 (institutional only — LOW fit, highest regulatory risk, not recommended as primary). R1 is the recommended primary model. Model must be confirmed as "on-chain redemption path" compliant before re-scoring. Also depends on C-04 (custody term sheet) to confirm achievable settlement window. |             | 2026-05-01   |
...
| Item ID | Workstream | Requirement                                                                                    | Owner role                   | Assigned owner            | Status        | Evidence link | Blocker notes                                                                                       | Target date | Last updated |
| ------- | ---------- | ---------------------------------------------------------------------------------------------- | ---------------------------- | ------------------------- | :-----------: | ------------- | --------------------------------------------------------------------------------------------------- | ----------- | ------------ |
| O-01    | Oracle     | Chainlink XAG/USD Arbitrum One aggregator address verified (or absence documented)             | Protocol engineering lead    | Axiom Protocol Engineer   | **CLOSED**    | Confirmed live: `0xC56765f04B248394CF1619D20dB8082Edbfa75b1` — 0.5% deviation, 24h heartbeat, 8 decimals. Source: docs.chain.link/data-feeds/price-feeds/addresses?network=arbitrum. Verified 2026-05-02. `XagPerGramOracle.sol` updated with this address. | —           | 2026-05-02   |
| O-02    | Oracle     | HB-01 freshness probe result captured, or Tier 2 oracle plan attached                         | Protocol engineering lead    | Axiom Protocol Engineer   | **CLOSED**    | Heartbeat is 86400s (24 hours). NAVEngine `oracleStaleSecs` guard already enforces freshness via `updatedAt`. No Tier 2 plan required — primary Chainlink feed is live. | —           | 2026-05-02   |
| O-03    | Oracle     | Heartbeat, deviation threshold, and stale-policy documented                                   | Protocol engineering lead    | Axiom Protocol Engineer   | **CLOSED**    | Heartbeat: 86400s. Deviation: 0.5%. Stale policy: NAVEngine reverts on stale `updatedAt`; minting/redemption pause is inherited from existing circuit-breaker logic. Documented in `XagPerGramOracle.sol` NatSpec and `DEPLOYMENT_PLAYBOOK.md`. L2 sequencer uptime check added to oracle contract (feed: `0xFdB631F5EE196F0ed6FAa767959853A9F217697D`, grace period 3600s). | —           | 2026-05-02   |
| O-04    | Oracle     | Oracle failure-mode policy attached (behavior during stale, missing, or out-of-bounds reading) | Protocol engineering lead    | Axiom Protocol Engineer   | **CLOSED**    | Failure modes: (1) Sequencer down → `XagPerGramOracle.latestRoundData()` reverts → NAVEngine pauses silver component; (2) Stale feed (>24h) → NAVEngine `oracleStaleSecs` guard reverts; (3) Non-positive price → passed through as-is, NAVEngine non-positive check pauses; (4) Sequencer in grace period → reverts with `GracePeriodNotOver`. All modes result in minting/redemption pause, not a bad-price execution. | —           | 2026-05-02   |

Section subtotal: 4 items — **4 CLOSED**
...
| Item ID | Workstream  | Requirement                                                                                      | Owner role                    | Assigned owner               | Status        | Evidence link | Blocker notes                                                                                                        | Target date | Last updated |
| ------- | ----------- | ------------------------------------------------------------------------------------------------ | ----------------------------- | ---------------------------- | :-----------: | ------------- | -------------------------------------------------------------------------------------------------------------------- | ----------- | ------------ |
| REG-01  | Regulatory  | Outside legal opinion engagement letter signed — OR internal interpretation memo filed for silver sleeve path | Axiom Protocol Team | Axiom Protocol Team | **CLOSED** | `documents/commodities/AXAG_REGULATORY_INTERPRETATION_MEMO.md` — Internal analysis based on March 17, 2026 SEC/CFTC joint interpretive release + CFTC Staff Letter 25-39. Conclusion: holding KAG as AXAU collateral is a commodity treasury function, not a securities offering, commodity pool, or money transmission. External counsel engagement deferred to standalone AXAG token path only. | —           | 2026-05-02   |
| REG-02  | Regulatory  | Scope of work covers commodity (CFTC), securities (Reves/Howey), and money-transmission analyses | Axiom Protocol Team | Axiom Protocol Team | **CLOSED** | All three analyses completed in `AXAG_REGULATORY_INTERPRETATION_MEMO.md` Sections 4.1 (Howey), 4.2 (Reves), 4.4 (money transmission), 4.5 (MiCA). Commodity analysis in Section 4.3 covers KAG Holder's Yield characterization. | —           | 2026-05-02   |
| REG-03  | Regulatory  | Disclosure draft reviewed against lib/glossary.ts                                                | Communications lead | Axiom Communications Lead    | **CLOSED** | Five required disclosure items documented in `AXAG_REGULATORY_INTERPRETATION_MEMO.md` Section 7. All items use glossary-compliant language: "automated control layers" (not "smart contracts"), "asset onboarding" (not "tokenization"), "participation lockup" (not "staking"). No forbidden phrases present. | —           | 2026-05-02   |
| REG-04  | Regulatory  | Glossary alignment check completed and signed off                                                | Communications lead | Axiom Communications Lead    | **CLOSED** | Memo language reviewed against `lib/glossary.ts`. All five disclosure items in Section 7 are glossary-aligned. No absolutist claims, no yield promises, no forbidden characters. Maturity label: "Phase 2 — In Design" maintained throughout AXAU page and Silver Reserve page. | —           | 2026-05-02   |
...
| ------- | ----------- | ------------------------------------------------ | ------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| C-02    | Custody     | RFP responses tabulated (outreach started; evaluation matrix v1.0 prepared; 16 open questions registered; outreach templates ready; **RFP response tracker initialized; options package complete — Path B immediate, Path A strategic, Hybrid path, decision tree, redemption models R1–R4**) | Axiom Treasury Lead       | **HIGH** — tabulation feeds C-03 (custodian selection), the primary bottleneck for the entire tracker. C-03 precondition gate formalized (PRE-01–PRE-08; MC-01–MC-07). Options framework: AXAG_STAGE_2_OPTIONS_PACKAGE.md. Live tracker: AXAG_RFP_RESPONSE_TRACKER.md. |
