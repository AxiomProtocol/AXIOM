...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const body = req.body as DisableUserRequest;
    ...
    if (targetRole && isPrivilegedRole(targetRole)) {
      if (!body.uniqueKey) {
        return res.status(400).json({
          error: 'Disabling privileged users requires a proposal. Use POST /api/admin/proposals/create with action_type=disable_privileged_user',
          requestId: actor.requestId,
        });
      }
      ...
      try {
        ...
        await client.query(
          `INSERT INTO admin_proposals 
           ...
           VALUES ($1, 'disable_privileged_user', 'user', $2, $3, 'pending', $4, $5, $6, $7, $8)`,
          [
            proposalId,
            body.userId,
            JSON.stringify({ targetUserId: body.userId, currentRole: targetRole }),
            body.reason,
            actor.userId,
            expiresAt,
            actor.requestId,
        ...
        return res.status(202).json({
          message: 'Disabling privileged user requires two-step approval',
          proposalId,
    ...
    await pool.query(
      `INSERT INTO admin_audit_log 
       ...
       VALUES ($1, $2, 'user_disabled', 'user', $3, $4, $5, $6, $7)`,
      [
        actor.userId,
        actor.role,
        body.userId,
    ...
    console.error(`[${actor.requestId}] Error disabling user:`, error);
