...
export default function CostIntelligencePanel({ dealId, propertyId, arvEstimate }: Props) {
  ...
  return (
    <div className="border border-dl-border">
      <div className="border-b border-dl-border px-4 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <div>
          ...
          <p className="font-dl-mono text-xs text-dl-muted">
            Craftsman NCE-grounded rehab estimate · {REGION_OPTIONS.find(r => r.code === setupForm.regionCode)?.label.split(' (')[0]}
          </p>
        </div>
        {estimates.length > 0 && (
          <select
        ...
        <div className="flex border-b border-dl-border">
          {(['setup', 'scope', 'review'] as PanelTab[]).map(t => (
            <button
              ...
              className={`px-4 py-2 font-dl-mono text-xs border-b-2 -mb-px capitalize ${
                panelTab === t
      ...
      <div className="p-4 sm:p-6">
        {loadingList ? (
          ...
          <div className="space-y-6">
            <div>
              <p className="font-dl-mono text-xs text-dl-muted uppercase mb-4 border-b border-dl-border pb-2">
                New Estimate Setup
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={setupForm.contingencyPct}
            ...
            <button
              ...
              className="bg-dl-navy text-white px-8 py-2 font-dl-mono text-sm"
            >
              Create Estimate
            </button>
          </div>
          ...
          <>
              ...
              <div>
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
                  <div>
                    ...
                    {currentEstimateMeta?.status === 'draft' && !scopeItems.length && (
                ...
                {genError && (
                  <p className="font-dl-mono text-xs text-red-600 mb-3">{genError}</p>
                )}
            ...
            {panelTab === 'review' && (
              <div>
                {loadingEstimate ? (
                  <p className="font-dl-mono text-xs text-dl-muted">Loading estimate...</p>
                ) : !estimate || !estimate.lineItems?.length ? (
                  <div>
                  ...
                  <div>
                    <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-4">
                      <div>
                        <p className="font-dl-mono text-xs text-dl-muted uppercase">
                          {estimate.estimateName || 'Estimate'} · v{estimate.version}
                        </p>
                      </div>
                      <div className="sm:ml-auto flex gap-2">
                        <button
                          onClick={() => setPanelTab('scope')}
                          className="border border-dl-border text-dl-muted px-4 py-2 font-dl-mono text-xs"
                        >
                          Edit Scope
                        </button>
                        <button
      ...
      {estimates.length > 0 && !selectedEstimateId && (
        <div className="border-t border-dl-border px-6 py-3">
          <button
            onClick={() => {
