...
export default function WalletBalanceStrip({ adminKey, balance, loading, topupLoading, onRefresh, onTopUp }: Props) {
  ...
  if (!adminKey) {
    return (
      <div className="border border-dl-border bg-dl-bg-alt px-5 py-3 mb-6 flex items-center justify-between">
  ...
  return (
    <div className="border border-dl-navy bg-white mb-6">
      ...
      <div className="px-5 py-3 border-b border-dl-navy bg-dl-navy flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="font-dl-mono text-xs uppercase tracking-wider text-white border border-white px-3 py-1 hover:bg-white hover:text-dl-navy transition-colors"
          >
      ...
      {/* Top-up panel (expandable) */}
      {showTopUp && (
        <div className="px-5 py-4 border-t border-dl-border bg-dl-bg-alt flex flex-wrap gap-3 items-center">
          <p className="font-dl-mono text-xs uppercase tracking-widest text-dl-gray mr-2">Top up via debit card:</p>
