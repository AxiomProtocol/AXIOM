      ...
      <div className="p-3 sm:p-4 border-b border-gray-200">
            ...
            <div className="flex items-center gap-2 sm:gap-4">
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-2 text-base sm:text-lg font-semibold text-gray-900 hover:text-teal-600 transition-colors"
                >
                  {selectedPair.name}
                ...
                {showPairSelector && (
                  <div className="absolute top-full left-0 mt-2 bg-gray-100 rounded-lg shadow-xl border border-gray-600 z-20 min-w-[180px]">
                    {TRADING_PAIRS.map((pair) => (
                      <button
                        ...
                        className={`w-full px-4 py-2 text-left text-sm hover:bg-gray-600 transition-colors first:rounded-t-lg last:rounded-b-lg ${
                          selectedPair.id === pair.id ? 'bg-teal-500/20 text-teal-600' : 'text-gray-900'
                        }`}
                      >
                        {pair.name}
                      </button>
              ...
              <div className="flex items-center gap-1 sm:gap-2">
                {loading ? (
                  <span className="text-lg sm:text-2xl font-bold text-gray-500">Loading...</span>
                ) : (
                  <>
                ...
                <button
                  ...
                  className={`px-2 sm:px-3 py-1 text-xs sm:text-sm rounded-md transition-colors ${
                    timeframe === tf
                      ...
                      : 'text-gray-300 hover:text-gray-900 disabled:opacity-50'
                  }`}
                >
                  {tf}
                </button>
        ...
        <div className="flex gap-3 sm:gap-6 mt-3 text-xs sm:text-sm overflow-x-auto">
          <div className="flex-shrink-0">
            <span className="text-gray-500">{timeframe} High:</span>
            <span className="ml-1 sm:ml-2 text-gray-900">{formatPrice(stats.high)}</span>
          </div>
          <div className="flex-shrink-0">
            <span className="text-gray-500">{timeframe} Low:</span>
            <span className="ml-1 sm:ml-2 text-gray-900">{formatPrice(stats.low)}</span>
          </div>
          <div className="flex-shrink-0">
            <span className="text-gray-500">Volume:</span>
            <span className="ml-1 sm:ml-2 text-gray-900">{formatVolume(stats.volume)}</span>
          </div>
        </div>
      </div>
