...
export const capActionTypeEnum = pgEnum('cap_action_type', [
  ...
  'UNSTAKE',
  'CUSTODY_MOVE',
  // Collateral admission action — gated by collateral_class on cap_assets and
  // by per-asset caps loaded from cap_risk_policies. See
  // documents/policies/collateral-risk-policy.md.
  'BORROW',
]);
...
// Collateral class — orthogonal to exposureClass. Governs whether an asset
// may be admitted as collateral (BORROW action) and under what caps.
//   GREEN  — fully approved, native, redeemable, oracle-stable.
//   YELLOW — approved with mandatory per-asset caps and isolated markets.
//   RED    — not admissible as collateral. Default for new assets (fail-closed).
// See documents/policies/collateral-risk-policy.md.
export const capCollateralClassEnum = pgEnum('cap_collateral_class', [
  'GREEN',
  'YELLOW',
  'RED',
]);
...
export const capAssets = pgTable('cap_assets', {
  ...
  status: capRecordStatusEnum('status').notNull().default('ACTIVE'),
  // Collateral admission class. Default RED is the fail-closed safety
  // invariant from the Collateral Risk Policy: no asset is borrowable until
  // explicitly classified GREEN or YELLOW with rationale.
  collateralClass: capCollateralClassEnum('collateral_class').notNull().default('RED'),
  collateralClassificationRationale: text('collateral_classification_rationale'),
  metadataJson: jsonb('metadata_json'),
  createdAt: timestamp('created_at').notNull().default(sql`now()`),
  updatedAt: timestamp('updated_at').notNull().default(sql`now()`),
}, (t) => ({
  ...
  collateralClassIdx: index('cap_assets_collateral_class_idx').on(t.collateralClass),
}));
 ...
 *     targetWalletAddress on Arbitrum One.
 *   - AXAU_MINT: reserved for future direct AXAU purchase via card.
 */
export const capCardDeposits = pgTable('cap_card_deposits', {
  id: varchar('id', { length: 40 }).primaryKey(),
  userId: varchar('user_id', { length: 40 }),
  ...
  idemUq: uniqueIndex('cap_card_deposits_idem_uq').on(t.idempotencyKey),
  sessionUq: uniqueIndex('cap_card_deposits_session_uq').on(t.stripeSessionId),
  statusIdx: index('cap_card_deposits_status_idx').on(t.status, t.createdAt),
  intentIdx: index('cap_card_deposits_intent_idx').on(t.intent, t.createdAt),
}));

/**
 * Append-only log of Stripe webhook events processed by the card-deposit
 * webhook. Stripe event id is unique to enforce idempotency at the
 * gateway level, independent of cap_card_deposits state.
 */
export const capCardDepositWebhookEvents = pgTable('cap_card_deposit_webhook_events', {
  id: varchar('id', { length: 40 }).primaryKey(),
  ...
  depositIdx: index('cap_card_deposit_webhook_events_deposit_idx').on(t.depositId),
}));
...
export const capBridgeAllowlistProposals = pgTable('cap_bridge_allowlist_proposals', {
  id: varchar('id', { length: 40 }).primaryKey(),
  // Symbol of the asset being proposed for allow-listing (e.g. WBTC).
  assetSymbol: varchar('asset_symbol', { length: 32 }).notNull(),
  // Free-form description of the bridge / wrapper provenance, e.g.
  // "WBTC issued by BitGo via the WBTC DAO mint/burn process".
  bridgeProvenance: text('bridge_provenance').notNull(),
  // On-chain address of the validity adapter that will be set in
  // CollateralRiskConfig. May be empty during DRAFT/COMMENT.
  validityAdapterAddress: varchar('validity_adapter_address', { length: 80 }),
  // Per-asset cap proposed (string for arbitrary precision; numeric in
  // base units of the asset).
  perAssetCap: numeric('per_asset_cap', { precision: 30, scale: 10 }),
  // Comment window end timestamp.
  commentWindowEndsAt: timestamp('comment_window_ends_at').notNull(),
  status: varchar('status', { length: 32 }).notNull(),
  // Vote tallies, recorded only after status moves to VOTE.
  yesVotes: integer('yes_votes').notNull().default(0),
  noVotes: integer('no_votes').notNull().default(0),
  // Author identifier (capUsers.id, optional during early phases).
  createdBy: varchar('created_by', { length: 40 }),
  createdAt: timestamp('created_at').notNull().default(sql`now()`),
  executedAt: timestamp('executed_at'),
  executedTxHash: varchar('executed_tx_hash', { length: 80 }),
  metadataJson: jsonb('metadata_json'),
}, (t) => ({
  statusIdx: index('cap_bridge_allowlist_status_idx').on(t.status, t.createdAt),
  symbolIdx: index('cap_bridge_allowlist_symbol_idx').on(t.assetSymbol),
}));

export const capBridgeAllowlistProposalComments = pgTable('cap_bridge_allowlist_proposal_comments', {
  id: varchar('id', { length: 40 }).primaryKey(),
  proposalId: varchar('proposal_id', { length: 40 }).notNull(),
  // Free-form commenter identifier (display name, ENS, wallet, email).
  // Public commenters need not have a capUsers row.
  commenter: varchar('commenter', { length: 200 }).notNull(),
  body: text('body').notNull(),
  createdAt: timestamp('created_at').notNull().default(sql`now()`),
}, (t) => ({
  proposalIdx: index('cap_bridge_allowlist_comments_proposal_idx').on(t.proposalId, t.createdAt),
}));

export const capLossCoverageClaims = pgTable('cap_loss_coverage_claims', {
  id: varchar('id', { length: 40 }).primaryKey(),
  claimantWallet: varchar('claimant_wallet', { length: 80 }).notNull(),
  // Optional contact email for status updates. Not displayed publicly.
  contactEmail: varchar('contact_email', { length: 200 }),
  // Free-form position reference (loan id, mint id, market id, etc.).
  positionRef: text('position_ref'),
  // Array of transaction hashes the claimant cites as evidence.
  txHashesJson: jsonb('tx_hashes_json'),
  // Free-form description of the event.
  description: text('description').notNull(),
  // Requested amount in USD cents.
  amountRequestedCents: integer('amount_requested_cents').notNull(),
  // Eligible draw category per loss-coverage-reserve-policy.md §3.
  eligibilityCategory: varchar('eligibility_category', { length: 64 }).notNull(),
  status: varchar('status', { length: 32 }).notNull(),
  reviewerNotes: text('reviewer_notes'),
  // Array of {url, label} evidence URLs.
  evidenceUrlsJson: jsonb('evidence_urls_json'),
  decidedAt: timestamp('decided_at'),
  decidedBy: varchar('decided_by', { length: 200 }),
  paidAmountCents: integer('paid_amount_cents'),
  paidTxHash: varchar('paid_tx_hash', { length: 80 }),
  paidAt: timestamp('paid_at'),
  createdAt: timestamp('created_at').notNull().default(sql`now()`),
  updatedAt: timestamp('updated_at').notNull().default(sql`now()`),
}, (t) => ({
  statusIdx: index('cap_lcc_status_idx').on(t.status, t.createdAt),
  walletIdx: index('cap_lcc_wallet_idx').on(t.claimantWallet),
}));

export const capLossCoverageClaimEvents = pgTable('cap_loss_coverage_claim_events', {
  id: varchar('id', { length: 40 }).primaryKey(),
  claimId: varchar('claim_id', { length: 40 }).notNull(),
  ...
  claimIdx: index('cap_lcc_events_claim_idx').on(t.claimId, t.createdAt),
}));
...
// Card onramp
export type CapCardDeposit = typeof capCardDeposits.$inferSelect;
...
export type NewCapBridgeAllowlistProposal = typeof capBridgeAllowlistProposals.$inferInsert;
export type CapBridgeAllowlistProposalComment = typeof capBridgeAllowlistProposalComments.$inferSelect;
export type NewCapBridgeAllowlistProposalComment = typeof capBridgeAllowlistProposalComments.$inferInsert;
export type CapLossCoverageClaim = typeof capLossCoverageClaims.$inferSelect;
export type NewCapLossCoverageClaim = typeof capLossCoverageClaims.$inferInsert;
export type CapLossCoverageClaimEvent = typeof capLossCoverageClaimEvents.$inferSelect;
export type NewCapLossCoverageClaimEvent = typeof capLossCoverageClaimEvents.$inferInsert;
