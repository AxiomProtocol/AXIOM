...
- Stellar Payments Rail (Layer 00 Extension) — Axiom Rail: Axiom Protocol's own Stellar SEP-10/24/31/38 anchor, settled via Increase (FDIC-insured ACH/Wire). No third-party anchor partnerships required. Core library: `lib/multichain/stellar/axiom-rail/` (AxiomRailService.ts, IncreaseSettlement.ts, types.ts, index.ts). API routes at `/api/axiom-rail/`: `health`, `auth` (SEP-10), `sep24/info`, `sep24/deposit`, `sep24/withdraw`, `sep24/transaction`, `sep31/info`, `sep38/info`, `sep38/prices`, `sep38/quote`. Existing `/api/stellar/` routes retained for Horizon network health, corridors, and direct USDC transfers. Anchor registry (`lib/multichain/stellar/types.ts`) now has 3 entries: `axiom-rail` (primary, mainnet), `testanchor` (testnet testing), `circle` (USDC settlement layer). Active anchor env var: `STELLAR_ACTIVE_ANCHOR=axiom-rail` (default fallback in code). SEP-10 signing keypair: public key `GBLOO5JUZQDP6JMIX26X5AC26QUNYFYMNT2CLAMGAWDU4HA4VG2IAVIY`, secret in `STELLAR_SIGNING_SECRET_KEY`. stellar.toml updated with `TRANSFER_SERVER_SEP0024`, `ANCHOR_QUOTE_SERVER`, `DIRECT_PAYMENT_SERVER` all pointing to `axiomprotocol.app/api/axiom-rail/`. Fee structure: $0.50 flat + 0.1% per transaction. Corridors: USDC→USD ACH, USDC→USD Wire, USD→USDC deposit. DB table: `stellar_payment_transfers` (26 columns). Feature flag: `ENABLE_STELLAR_PAYMENTS_RAIL=true`.
