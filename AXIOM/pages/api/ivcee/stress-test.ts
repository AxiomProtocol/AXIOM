...
import { computeStressTests } from '../../../lib/ivcee/models';
import { insertStressRows } from '../../../lib/ivcee/persist';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const results = computeStressTests(input);
    await insertStressRows(dealId, scenarioId, results);

    return res.status(200).json({ data: results, meta: { dealId, scenarioId, computedAt: new Date().toISOString() } });
  } catch (err: any) {
    console.error('[IVCEE stress-test]', err.message);
