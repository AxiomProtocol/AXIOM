...
import { computeSensitivityMatrix } from '../../../lib/ivcee/models';
import { insertSensitivityRows } from '../../../lib/ivcee/persist';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const rows = computeSensitivityMatrix(input);
    await insertSensitivityRows(dealId, scenarioId, rows);

    return res.status(200).json({ data: { rows, count: rows.length }, meta: { dealId, scenarioId, computedAt: new Date().toISOString() } });
  } catch (err: any) {
    console.error('[IVCEE sensitivity]', err.message);
