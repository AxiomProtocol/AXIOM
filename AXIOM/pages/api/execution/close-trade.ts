...
import { closeExecution } from '../../../server/services/execution/executionService';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { executionId, closePrice, closeReason } = req.body;
  if (!executionId) {
    return res.status(400).json({ error: 'executionId is required' });
  }

  try {
    const result = await closeExecution(
      executionId,
    ...
    console.error('[execution/close-trade] Error:', err);
