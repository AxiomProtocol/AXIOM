...
contract AXIOMOracleAdapter {
    ...
    function _axusdToUsdc(uint256 inAxusd) internal view returns (uint256) {
        ...
        // Hardened by Task #210: when a PSM is CONFIGURED, an unreadable
        // PSM (returns 0,0 from _psmRate) MUST revert rather than silently
        // collapse to a 1:1 quote. Silent collapse is exploitable when
        // the PSM is in fact under-collateralised.
        if (psmNumer == 0 || psmDenom == 0) {
            require(primaryPsm == address(0), "AXIOMOracle: PSM read failed");
            return baseOut;
        }
    ...
    function _psmRate() internal view returns (uint256 numer, uint256 denom) {
        // ── Task #210 hardening ──────────────────────────────────────────────
        // Two distinct modes:
        //   (a) primaryPsm == address(0) — PSM intentionally unconfigured
        //       (e.g. unit-test fixture). Return neutral (1, 1) so callers
        //       can short-circuit to a 1:1 quote.
        //   (b) primaryPsm != address(0) — PSM is configured and SHOULD be
        //       readable. Any read failure, zero balance, or zero supply
        //       MUST surface as (0, 0) so the upstream callers
        //       (`_axusdToUsdc`, `_usdcToAxusd`, `axusdUsdPrice`) can
        //       revert with "AXIOMOracle: PSM read failed". Returning
        //       (1, 1) on a configured-but-unreadable PSM was the
        //       silent-fallback exploit path: it priced AXUSD at parity
        //       even when the PSM was under-collateralised.
        if (primaryPsm == address(0)) return (1, 1);
        try IERC20Decimals(USDC).balanceOf(primaryPsm) returns (uint256 psmUsdc) {
            if (psmUsdc == 0) return (0, 0);
            try IERC20Decimals(primaryAxusd).totalSupply() returns (uint256 supply18) {
                if (supply18 == 0) return (0, 0);
                uint256 supply6 = supply18 / 1e12;
                if (supply6 == 0) return (0, 0);
