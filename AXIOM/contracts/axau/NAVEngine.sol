...
contract NAVEngine is INAVEngine {
    ...
    /**
     * @notice Set the stale-oracle behaviour.
     *
     * @dev    Hardened by Task #210 (Collateral Exploit Prevention Framework):
     *         the unsafe `false` path is no longer reachable. Calling with
     *         `revertOnStale = false` reverts. The toggle is retained so
     *         existing governance scripts that re-assert the safe value
     *         continue to work and emit the standard ConfigUpdated event.
     *
     *         Doctrine: NAV computation MUST never silently degrade to
     *         zero on stale or non-positive oracle answers. A consumer
     *         that needs partial valuation should compute it explicitly,
     *         not rely on the engine swallowing the failure.
     */
    function setRevertOnStaleOracle(bool revertOnStale) external onlyGovernor {
        require(revertOnStale, "NAVEngine: revertOnStaleOracle cannot be disabled");
        revertOnStaleOracle = true;
        emit ConfigUpdated("revertOnStale", 1);
    }
