...
/// @notice Minimal local view of the Task #210 CollateralGuard. When the
///         guard is unset (address(0)), pre-#210 behaviour is preserved.
interface ICollateralGuardMR {
...
contract MintRedeemController {
    ...
    /// @notice Optional fail-closed guard called on every mintWithAsset.
    ///         When unset (address(0)), pre-#210 behaviour is preserved.
    ICollateralGuardMR public collateralGuard;
    /// @notice Stable market identifier passed to the guard. Set once.
    bytes32 public mintMarketId;
    /// @notice Aggregated reserve-asset exposure (in reserve-asset native
    ///         decimals) per vaultId. Used for the guard's cap check.
    mapping(bytes32 => uint256) public mintedReserveByVault;
        ...
        returns (uint256 axauOut)
    {
        ...
        // ── Task #210: fail-closed collateral admission for the mint path ──
        // When the CollateralGuard is wired, EVERY mint MUST pass the
        // guard. The guard composes (a) global/per-market halt, (b) asset
        // enabled, (c) validity adapter (default-deny for bridged/wrapped
        // /receipt assets), (d) per-asset cap ceiling. Pre-wire (guard
        // == address(0)) behaviour is unchanged.
        if (address(collateralGuard) != address(0)) {
            address _asset = IVault(comp.vault).reserveAsset();
            uint256 _outstanding = mintedReserveByVault[vaultId];
            ...
            mintedReserveByVault[vaultId] = _outstanding + tokenAmount;
        }
    ...
    /// @notice Wire the Task #210 CollateralGuard. Pass `address(0)` to
    ///         disable guard checks (back-compat). `_marketId` is the
    ///         stable identifier used by the IncidentController for
    ///         per-market halts of the AXAU mint path.
    function setCollateralGuard(address _guard, bytes32 _marketId) external onlyGovernor {
        collateralGuard = ICollateralGuardMR(_guard);
        mintMarketId = _marketId;
