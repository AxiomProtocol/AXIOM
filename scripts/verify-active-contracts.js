...
const FEE_PLUMBING_TX = '0xf978ae0331445de5f98fdc863eb2c1cdd5298c8a9b065d1205726edaffae5966';
...
async function run() {
  ...
  if (eulerAsset.toLowerCase() !== routerAxusd.toLowerCase()) {
    console.error(`FAIL: Euler binding mismatch — EulerVault.asset() (${eulerAsset}) !== RevenueRouter.axusd() (${routerAxusd})`);
    process.exit(1);
  }
  ...
  let primaryPsm = axiomPsmCandidates.find(c => c.label.includes('GENIUS'));
  if (!primaryPsm) {
    primaryPsm = axiomPsmCandidates.find(c => !c.label.includes('Original')) || axiomPsmCandidates[0];
  }

  let eulerPsm = axiomPsmCandidates.find(c => c.label.includes('Original'));
  if (!eulerPsm) {
    eulerPsm = axiomPsmCandidates.find(c => c.address.toLowerCase() !== primaryPsm.address.toLowerCase()) || axiomPsmCandidates[0];
  }

  if (primaryPsm.address.toLowerCase() === eulerPsm.address.toLowerCase()) {
    console.error('FAIL: PSM mapping error — both PSMs resolved to same address');
    process.exit(1);
  }
  ...
  console.log('='.repeat(72));
  console.log('  --- ON-CHAIN EVIDENCE (Blockscout) ---');
  console.log('='.repeat(72));
  console.log('');
  console.log('  PRIMARY AXUSD:');
  console.log(`    Latest TX Hash: ${primaryAxusd.lastTxHash || 'N/A'}`);
  console.log(`    Latest TX Timestamp: ${primaryAxusd.lastTxTimestamp || 'N/A'}`);
  console.log('');
  console.log('  PRIMARY PSM:');
  console.log(`    Latest TX Hash: ${primaryPsm.lastTxHash || 'N/A'}`);
  console.log(`    Latest TX Timestamp: ${primaryPsm.lastTxTimestamp || 'N/A'}`);
  console.log('');
  console.log('  EULER AXUSD:');
  console.log(`    Latest TX Hash: ${eulerAxusdResult.lastTxHash || 'N/A'}`);
  console.log(`    Latest TX Timestamp: ${eulerAxusdResult.lastTxTimestamp || 'N/A'}`);
  console.log('');
  console.log('  EULER PSM:');
  console.log(`    Latest TX Hash: ${eulerPsm.lastTxHash || 'N/A'}`);
  console.log(`    Latest TX Timestamp: ${eulerPsm.lastTxTimestamp || 'N/A'}`);
  console.log('');
  console.log(`  Fee plumbing configuration tx: ${FEE_PLUMBING_TX}`);
  console.log('');
